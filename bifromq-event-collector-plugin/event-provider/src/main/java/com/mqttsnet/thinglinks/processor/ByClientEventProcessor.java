 ...
 */
@Slf4j
public class ByClientEventProcessor extends AbstractEventProcessor {
    ...
    protected Map<String, Object> buildEventData(Event<?> event) {
        ...
        // 验证事件类型
        if (!(event instanceof ByClient)) {
            log.error("Invalid event type. Expected ByClient, but got {}", event.getClass().getSimpleName());
            return data;
        }
        // 构建事件数据
        Optional.of(event)
                ...
                .ifPresent(byClient -> {
                    data.put(CommonConstants.TENANT_ID, getSafeValue(byClient.clientInfo().getTenantId(), ""));
                    data.put("withoutDisconnect", getSafeValue(byClient.withoutDisconnect(), false));
                    Optional<Map<String, String>> metadataMapOpt = Optional.of(byClient.clientInfo().getMetadataMap());
