 ...
 */
@Slf4j
public class KickedEventProcessor extends AbstractEventProcessor {
    ...
    protected Map<String, Object> buildEventData(Event<?> event) {
        ...
        // 确保事件是 Kicked 类型
        if (!(event instanceof Kicked)) {
            log.error("Invalid event type. Expected Kicked, but got {}", event.getClass().getSimpleName());
            return data;
        }
        ...
        Optional.of(event)
                ...
                .ifPresent(kicked -> {
                    data.put(CommonConstants.TENANT_ID, getSafeValue(kicked.clientInfo().getTenantId(), ""));

                    Optional<Map<String, String>> metadataMapOpt = Optional.of(kicked.clientInfo().getMetadataMap());
